...
## 功能
- 确定性地形生成（山丘/海洋/沙滩/橡树/矿物/村庄），256×256×128 固定世界
- **生存模式**：生命/饥饿/摔落/溺水/死亡重生、背包（E）、合成（2×2 背包 + 3×3 工作台）、工具耐久与方块硬度、连续挖掘（长按）+ 裂纹/粒子、掉落与拾取、6 种矿物、熔炉烧矿、F 键作弊切换创造
- 创造模式：飞行、即时破坏、无限放置、9 格热栏 + E 方块选择器
- 村民 NPC + 程序生成小村庄（头部贴用户上传图片）
- WebRTC 点对点联机：建房（5 位房间码）或加入，同步方块/掉落/玩家/村民，聊天
- 房主本地存档：方块/背包/熔炉/模式自动保存，重新建房可选载入
